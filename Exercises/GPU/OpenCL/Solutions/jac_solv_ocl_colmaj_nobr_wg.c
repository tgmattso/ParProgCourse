...
**
**  WARNINGS:
**           1. Make sure TYPE is the same in mm_utils.h and the kernel
**           2. On OSX for a CPU, set WGSIZE to 1
**           3. For the Gen GPU, TYPE must be float
**
...
//
  ...
  char *kernel_string = get_kernel_string("jac_ocl_colmaj_nobr.cl");
...
void parse_arguments(int argc, char *argv[])
{
  for (int i = 1; i < argc; i++)
  {
    ...
    {
      if (++i >= argc || !parse_uint(argv[i], &wgsize))
      {
    ...
    else if (!strcmp(argv[i], "--help") || !strcmp(argv[i], "-h"))
    {
      ...
      printf("        --wg         WGSIZE  Set workgroup size to WGSIZE\n");
